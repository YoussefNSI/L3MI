...
function extraireCommunes(communes, région, bloc) {
    ...
    query.append("région", région);
    query.append("bloc", bloc);
    let header = {
        method: 'POST',
        headers: {
    ...
    return fetch('communes.php', header)
        .then((réponse) => réponse.json())
        .then((bloc_communes) => {
            console.debug("bloc", bloc);
            console.debug("bloc_communes", bloc_communes);
            console.debug("test", bloc_communes.length != 0);
            if (bloc_communes.length != 0) {
                return extraireCommunes(communes.concat(bloc_communes), région, bloc + 1);
            } else {
                return communes;
            }
        })
