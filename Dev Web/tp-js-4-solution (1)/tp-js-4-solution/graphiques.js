...
p_températures_anjou
    .then(relevés => {
        // marques
        let Tmin = Math.min(...relevés.map(relevé => relevé.tmin));
        let Tmax = Math.max(...relevés.map(relevé => relevé.tmax));
        // tracé
        const plot = Plot.plot({
            title: "Températures mensuelles angevines en 2022",
            color: {
...
document.querySelector("select").addEventListener("change", function(e) {
    ...
    p_communes(région)
        .then(communes_région => {
            ...
            let communes = communes_région.map((commune) =>
                `${commune["région"]}/${commune["département"]}/${commune["nom"]}`);

            console.debug("|communes|", communes.length);
            // tracé
            const plot = Plot.plot({
                axis: null,
                width: 800,
            ...
            if (div.lastElementChild) {
                div.lastElementChild.replaceWith(plot);
            } else {
