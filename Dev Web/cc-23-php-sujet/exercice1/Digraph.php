...
class Digraph extends ADigraph {
    ...
    {
        return in_array($nodeId, $this->getNodeIds(), true);
    }
    ...
    {
        if(array_keys($arc) !== [0, 1]) {
            throw new InvalidArgumentException("Each arc must be an array indexed over 0..1");
        }
        if(! $this->checkNodeId($arc[0]) || ! $this->checkNodeId($arc[1])) {
            throw new InvalidArgumentException("The tail of an arc must be in range [[V]]");
        }
        if(! is_int($arc[0]) || ! is_int($arc[1])) {
            throw new InvalidArgumentException("Each arc must be an array of integers indexed over 0..1");
        }
        return true;
    }
